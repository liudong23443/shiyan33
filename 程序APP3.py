        ...
        with st.spinner("计算预测结果..."):
            try:
                ...
                fig = go.Figure(go.Indicator(
                    ...
                    domain = {'x': [0, 1], 'y': [0, 1]},
                    title = {'text': "三年内死亡风险", 'font': {'size': 14, 'family': 'SimHei', 'color': 'black', 'weight': 'bold'}},
                    gauge = {
                ...
                fig.update_layout(
                    ...
                    paper_bgcolor="white",
                    plot_bgcolor="white",
                    font={'family': "SimHei", 'color': 'black', 'size': 11},
                )
                ...
                <div style="text-align: center; margin: -0.2rem 0 0.3rem 0;">
                    <span style="font-size: 1.1rem; font-family: 'SimHei'; color: {risk_color}; font-weight: bold;">
                        {risk_category}
                    </span>
                </div>
                """, unsafe_allow_html=True)
                ...
                try:
                    with st.spinner("正在生成SHAP解释图..."):
                        ...
                        # 手动修改标题和坐标轴标签 - 确保中文显示
                        plt.title("特征对预测的影响", fontsize=12, fontname='SimHei', fontweight='bold', color='black')
                        ...
                        feature_translations = {
                            "Intraoperative Blood Loss": "术中出血量",
                            "CEA": "癌胚抗原",
                            "Albumin": "白蛋白",
                            "TNM Stage": "TNM分期",
                            "Age": "年龄",
                            "Max Tumor Diameter": "肿瘤直径",
                            "Lymphovascular Invasion": "淋巴血管侵犯"
                        }
                        
                        # 尝试修改文本标签
                        for text in plt.gcf().findobj(plt.Text):
                            for eng_name, cn_name in feature_translations.items():
                                if eng_name in text.get_text():
                                    text.set_text(text.get_text().replace(eng_name, cn_name))
                        ...
                        <div style="background-color: #f0f7ff; padding: 5px; border-radius: 3px; margin-top: 3px; font-size: 0.8rem; border: 1px solid #dce8fa;">
                          <p style="margin:0"><strong>图表解释:</strong> 上图显示了各特征对预测的影响。红色表示增加死亡风险，蓝色表示降低死亡风险。</p>
                        </div>
                        """, unsafe_allow_html=True)
